...
| 会員アクセス（Magic Link 完了後） | https://gatame-kosen-learning-pass-navigato.vercel.app/dashboard |
| Kajabi からの導線例 | `https://gatame-kosen-learning-pass-navigato.vercel.app/?email={{member.email}}` |
| **Kajabi（推奨・自動 OTP）** | `https://gatame-kosen-learning-pass-navigato.vercel.app/access?email={{member.email}}` → `AccessPage` が `signInWithOtp` を自動実行 |
...
`application.yml` の CORS デフォルトは上記 Vercel ドメインと一致済み。Supabase の **Site URL** および **Redirect URLs** に、フロント URL と `/dashboard` を登録すること。

## 9. Vercel（フロント）と API プロキシ
