...
| **診断（要ログイン）** | https://gatame-kosen-learning-pass-navigato.vercel.app/diagnostic |
| **会員アクセス（未ログインの入り口）** | https://gatame-kosen-learning-pass-navigato.vercel.app/access |
| Kajabi（推奨） | `https://gatame-kosen-learning-pass-navigato.vercel.app/access?email={{member.email}}` |
| ルート `/` + email | 未ログイン時は自動で `/access?email=...` へリダイレクト |
