...
// --------------------------------------------------------------------------
// 4. モジュール完了フィードバック (module_feedback) — Member のみ
// --------------------------------------------------------------------------

export type ModuleFeedbackDifficulty = 'TOO_EASY' | 'JUST_RIGHT' | 'TOO_HARD'

export async function saveRemoteModuleFeedback(
  userId: string,
  params: {
    moduleId: string
    difficulty: ModuleFeedbackDifficulty
    satisfaction: number
    comment?: string
  },
): Promise<SyncWriteResult> {
  ...
  if (!client || !userId) return { ok: false, message: 'Supabase is not configured' }
  try {
    const { error } = await client.from('module_feedback').insert({
      user_id: userId,
    ...
    if (error) return writeFailure(error, 'Failed to save module feedback')
    return { ok: true }
  } catch (e) {
    return { ok: false, message: e instanceof Error ? e.message : 'Network error' }
  }
