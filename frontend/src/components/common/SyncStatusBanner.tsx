...
export default function SyncStatusBanner() {
  const { status, pendingCount, retryNow } = useSync()
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`fixed inset-x-0 top-0 z-[100] flex flex-wrap items-center justify-center gap-3 border-b px-4 py-2.5 text-center text-sm shadow-lg backdrop-blur-sm ${toneClass}`}
    >
      <span>
        {copy}
